...
class TranscriptionManager:
    ...
    def transcribe_audio(
        ...
        if transcript_path.exists() and not self.config.get('force_reprocess', False):
        ...
        if transcript_path.exists() and self.config.get('force_reprocess', False):
        ...
        try:
            ...
            logger.error(f"Error getting file size for {audio_path}: {e}")
            file_size_mb = 0
        if file_size_mb > max_size_mb:
            ...
            json_path = transcript_path.with_suffix('.segments.json')
            ...
            if all_words:
                srt_path = Path(self.file_manager.get_subtitle_path(file_id, 'orig'))
    ...
    def get_transcript_text(self, file_id: str) -> Optional[str]:
        ...
        # Get transcript path using Path object for robust path handling
        transcript_path = Path(self.file_manager.get_transcript_path(file_id))
        ...
        try:
            # Use Path.read_text() for simpler file reading
