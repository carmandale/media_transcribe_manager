#!/usr/bin/env python3
"""
Evaluate English translation quality via OpenAI GPT-4

Given a file UUID, this script loads the original transcript and English candidate translation
from the standard output directories and asks an OpenAI model to rate the English quality.

Usage
-----
python scripts/evaluate_english_quality.py --uuid <FILE_UUID> [--model gpt-4.1]

Returned JSON example
---------------------
{
  "score_0_to_10": 9,
  "issues": ["Minor phrasing issue"],
  "overall_comment": "Good translation, minor style adjustments needed."
}
"""
...
PROMPT_TEMPLATE = (
    ...
    "Evaluate the **English** candidate translation against the original transcript.\n\n"
    "Give a holistic score from 0 (unintelligible) to 10 (publication quality). "
    "Focus on accuracy, grammar, style, and terminology.\n\n"
...
def build_paths(uuid: str) -> Dict[str, pathlib.Path]:
    ...
    base_transl = pathlib.Path("./output/translations") / "en"
    ...
    # English translation
    transl_matches = list(base_transl.glob(f"{uuid}*_{'en'}.txt"))
    if not transl_matches:
        raise FileNotFoundError(f"No English translation file found for {uuid}")
    paths["en"] = transl_matches[0]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate English translation quality.")
