...
class Database:
    ...
    def _initialize_schema(self):
        ...
        with self.transaction() as conn:
            ...
            conn.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_quality_evaluations_language 
                ON quality_evaluations(language)
            """
            )
            conn.execute(
                """
                CREATE INDEX IF NOT EXISTS idx_quality_evaluations_file_language 
                ON quality_evaluations(file_id, language)
            """
            )
    ...
    def add_file_simple(self, file_path: Union[str, Path]) -> Optional[str]:
        ...
        media_type = (
            "video" if file_path.suffix.lower() in video_extensions else "audio"
        )
    ...
    def update_status(
        self, file_id: str, status: Optional[str] = None, **stage_statuses
    ) -> bool:
        """
    ...
    def get_pending_files(
        self, stage: str, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_files_by_status(
        self, status: Union[str, List[str]], limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
